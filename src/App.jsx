                            ...
                            unsubscribeProfile = onSnapshot(profileDocRef, (docSnap) => {
                                if (docSnap.exists()) {
                                    ...
                                    setDoc(profileDocRef, {
                                        ...
                                        skillsToLearn: [],
                                        skillsToTeach: [],
                                        groupChatClearedAt: null // New field for group chat clearing
                                    }, { merge: true }).then(() => {
                                        setUserProfile({
                                            name: defaultName,
                                            skillPreferences: '',
...
const DailyAdvice = () => {
    ...
    const generateAdvice = async () => {
        ...
        try {
            ...
            const apiKey = ""; // New Gemini API Key
            ...
            if (result.candidates && result.candidates.length > 0 &&
                ...
                // Fix: Robustly get the last advice date string
                const lastAdviceDateValue = userProfile?.lastAdviceDate;
                const lastAdviceDateString = lastAdviceDateValue
                    ? (lastAdviceDateValue.toDate ? lastAdviceDateValue.toDate().toDateString() : (lastAdviceDateValue instanceof Date ? lastAdviceDateValue.toDateString() : null))
                    : null;

                let newStreak = userProfile?.dailyStreak || 0;
                let newPoints = (userProfile?.points || 0) + 10;

                if (lastAdviceDateString !== today) { // Use the corrected variable
                    const yesterday = new Date();
                    yesterday.setDate(yesterday.getDate() - 1);
                    const yesterdayString = yesterday.toDateString();

                    if (lastAdviceDateString === yesterdayString) { // Use the corrected variable
                        newStreak += 1;
                    } else {
                        newStreak = 1;
                    }
                    ...
                    setUserProfile(prev => ({
                        ...
                        points: newPoints,
                        dailyStreak: newStreak,
                        lastAdviceDate: new Date() // Still set as JS Date for immediate local state consistency
                    }));
                } else {
...
const Chatbot = () => {
    ...
    const sendMessage = async () => {
        // Corrected logic for disabled state, check if input or image exists
            ...
            <div className="bg-white bg-opacity-90 p-4 rounded-lg"> {/* Added wrapper for readability */}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input
                        ...
                        // Ensure input is not disabled unless chat is loading or auth is not ready
                        disabled={loadingChat || loadingAuth || !chatHistoryReady}
                    />
                    <button
                        onClick={sendMessage}
                        ...
                        // Disable send button if no input or image, or if loading/not ready
...
const PersonalizationAI = () => {
    ...
    const [userAnswers, setUserAnswers] = useState({}); // Changed to object for easier answer mapping
...
---`;
    ...
    const parseQuizResponse = (rawText) => {
        const questions = [];
        // The regex ensures '---' is treated as a delimiter only if it's on its own line
...
const GroupChat = () => {
    const { db, userId, loadingAuth, userProfile, setUserProfile } = useAuth(); // Added setUserProfile
    const [messages, setMessages] = useState([]);
    const [input, setInput] = useState('');
    ...
    const [confirmDeleteMessageId, setConfirmDeleteMessageId] = useState(null); // State for message to delete
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Filter messages based on user's cleared timestamp
            const clearedAt = userProfile?.groupChatClearedAt?.toDate(); // Convert Firestore Timestamp to JS Date
            if (clearedAt) {
                setMessages(fetchedMessages.filter(msg => msg.timestamp?.toDate() > clearedAt));
            } else {
                setMessages(fetchedMessages);
            }
            setLoadingChat(false);
        ...
        return () => unsubscribe();
    }, [db, userId, loadingAuth, userProfile?.groupChatClearedAt]); // Added userProfile.groupChatClearedAt as dependency
    ...
    const confirmClearMessages = async () => {
        ...
        setMessage('Clearing chat for your view...');
        ...
        const userProfileRef = doc(db, `artifacts/${getAppId()}/users/${userId}/profile/userProfile`);
        try {
            await updateDoc(userProfileRef, {
                groupChatClearedAt: serverTimestamp()
            });
            // Update local state immediately
            setUserProfile(prev => ({
                ...prev,
                groupChatClearedAt: new Date() // Use a Date object for local state consistency
            }));
            setMessage('Chat cleared for your view successfully!');
            setTimeout(() => setMessage(''), 3000);
        } catch (error) {
            setMessage('Error clearing chat for your view: ' + error.message);
            console.error("Error updating group chat cleared timestamp:", error);
        }
    };
    ...
    const confirmDeleteMessage = async () => {
        if (!confirmDeleteMessageId || !db || loadingAuth) return;

        const appId = getAppId();
        const messageDocRef = doc(db, `artifacts/${appId}/public/data/groupChats/${chatChannelId}/messages`, confirmDeleteMessageId);
        ...
        <div className="mt-8 p-6 bg-gradient-to-br from-teal-50 to-cyan-50 rounded-lg shadow-lg max-w-4xl mx-auto animate-fade-in">
            <div className="bg-white bg-opacity-90 p-4 rounded-lg"> {/* Added wrapper for readability */}
                ...
                {confirmClear && (
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg shadow-xl text-center transform transition-transform duration-300 animate-bounce-in-fade-out">
                            <p className="mb-4 text-lg font-semibold">Are you sure you want to clear all chat messages for *your view*?</p>
                ...
                {confirmDeleteMessageId && (
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg shadow-xl text-center transform transition-transform duration-300 animate-bounce-in-fade-out">
                            <p className="mb-4 text-lg font-semibold">Are you sure you want to delete this message?</p>
                            <div className="flex justify-center space-x-4">
                                <button
                    ...
                    {loadingChat ? (
                        ...
                        messages.map((msg, index) => (
                            <div key={msg.id || index} className={`flex ${msg.userId === userId ? 'justify-end' : 'justify-start'}`}>
                                <div
                                    className={`p-3 rounded-lg max-w-[75%] break-words relative group ${
                                        msg.userId === userId
                                ...
                                >
                                    ...
                                    {msg.userId === userId && (
                                        <button
                                            ...
                                            className="absolute -right-8 top-1/2 -translate-y-1/2 p-1 text-red-300 hover:text-red-700 transition-colors opacity-0 group-hover:opacity-100"
                ...
                <button
                    ...
                    disabled={loadingAuth || loadingChat}
                >
                    Clear All Chat Messages (My View)
                </button>
            </div>
        </div>
    );
...
const PrivateChatModal = ({ partnerUserId, partnerUserName, onClose, onMarkRead, onSendMessage }) => {
    ...
    const confirmDeleteMessage = async () => {
        ...
        const chatRoomId = getPrivateChatRoomId(userId, partnerUserId);
        const appId = getAppId();
        const messageDocRef = doc(db, `artifacts/${appId}/privateChats/${chatRoomId}/messages`, confirmDeleteMessageId);

        try {
            await deleteDoc(messageDocRef);
            setMessageError('Message deleted successfully!');
            setTimeout(() => setMessageError(''), 3000);
        } catch (error) {
            setMessageError('Error deleting message: ' + error.message);
            console.error("Error deleting message:", error);
        } finally {
            setConfirmDeleteMessageId(null);
        }
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 animate-fade-in">
            <div className="bg-gradient-to-br from-green-100 to-white p-6 rounded-lg shadow-xl w-full max-w-md mx-4 my-8 flex flex-col h-[80vh] transform transition-transform duration-300 hover:scale-[1.01]">
                <div className="bg-white bg-opacity-90 p-4 rounded-lg flex flex-col h-full">
                    ...
                    <div
                        ref={chatContainerRef}
                        className="bg-gray-50 p-4 rounded-lg border border-gray-200 overflow-y-auto flex flex-col space-y-3 custom-scrollbar flex-grow mb-4"
                    >
                        {loadingChat ? (
                                ...
                                <div key={msg.id || index} className={`flex ${msg.senderId === userId ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    >
                                        ...
                                        {msg.senderId === userId && (
                                            <button
                                                onClick={() => handleDeleteMessageClick(msg.id)}
                                                className="absolute -right-8 top-1/2 -translate-y-1/2 p-1 text-red-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                                                title="Delete message"
                                            >
                                                <i className="fas fa-trash"></i>
                                            </button>
...
// NEW: UserPresenceList Component for displaying active and inactive users
const UserPresenceList = ({ setShowPrivateChat, setPrivateChatWithUserId, setPrivateChatWithUserName }) => {
    const { db, userId, loadingAuth, userProfile } = useAuth();
    const [allUsersPresence, setAllUsersPresence] = useState([]);
    ...
    useEffect(() => {
        ...
        // Listener for all user presence data
        const q = query(userPresenceCollectionRef);
        const unsubscribeSnapshot = onSnapshot(q, (snapshot) => {
            const fetchedPresenceData = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            })).sort((a, b) => (a.userName || '').localeCompare(b.userName || '')); // Sort alphabetically by name

            setAllUsersPresence(fetchedPresenceData);
        }, (error) => {
            console.error("Error fetching all user presence:", error);
        });
    ...
    const now = Date.now();
    const activeUsers = allUsersPresence.filter(user =>
        user.id !== userId && user.isOnline && (now - (user.lastActive?.toDate ? user.lastActive.toDate().getTime() : 0) < 15000)
    );
    const inactiveUsers = allUsersPresence.filter(user =>
        user.id !== userId && (!user.isOnline || (now - (user.lastActive?.toDate ? user.lastActive.toDate().getTime() : 0) >= 15000))
    );
        ...
        <div className="bg-gradient-to-br from-green-50 to-blue-50 p-4 rounded-lg shadow-md border border-gray-200 mt-6 animate-fade-in transform transition-transform duration-300 hover:scale-[1.01]">
            <div className="bg-white bg-opacity-90 p-4 rounded-lg">
                <div className="mb-4 text-center">
                    <p className="text-md font-semibold text-gray-800">Current User: <span className="text-blue-600">{userProfile?.name || 'Loading...'}</span></p>
                </div>
                ...
                {activeUsers.length === 0 ? (
                    ...
                    <ul className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar"> {/* Active Users List */}
                ...
                <h4 className="text-lg font-semibold text-gray-700 mt-6 mb-3">Inactive Users ({inactiveUsers.length})</h4>
                {inactiveUsers.length === 0 ? (
                    <p className="text-gray-500 italic text-sm">No inactive users found.</p>
                ) : (
                    <ul className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar"> {/* Inactive Users List */}
...
const CommunityForum = ({ setShowPrivateChat, setPrivateChatWithUserId, setPrivateChatWithUserName }) => {
    ...
    // Handle image file selection for a specific comment
    const handleCommentImageChange = (postId, e) => {
        ...
        if (file) {
            ...
            reader.onerror = (error) => {
                console.error("Error reading comment image file:", error);
    ...
    const clearCommentImage = (postId) => {
        setCommentImages(prev => ({ ...prev, [postId]: null }));
        // Also clear the file input for that specific post/comment area
        const inputElement = document.getElementById(`commentImageUpload-${postId}`);
        if (inputElement) {
            inputElement.value = '';
        }
    };
    ...
    const handleAddComment = async (postId) => {
        ...
        if ((!commentContent?.trim() && !commentImage) || !userId || !db || loadingAuth || !userProfile) {
            setMessage('Please enter a comment or select an image to add.');
            return;
        }
        ...
        try {
            await addDoc(commentsCollectionRef, {
                text: commentContent?.trim() || '', // Ensure text is string, even if empty
                userId: userId,
                userName: userName,
                timestamp: serverTimestamp(),
                imageUrl: commentImage || null // FIX: Ensure imageUrl is null if commentImage is undefined
            });
            ...
            clearCommentImage(postId); // Clear image input
            ...
            <div className="bg-white bg-opacity-90 p-4 rounded-lg"> {/* Added wrapper for readability */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-1">
                        <UserPresenceList setShowPrivateChat={setShowPrivateChat} setPrivateChatWithUserId={setPrivateChatWithUserId} setPrivateChatWithUserName={setPrivateChatWithUserName} />
                    </div>
                </div>
                                    ...
                                    <div className="mt-3 pt-3 border-t border-gray-100 flex items-center space-x-4">
                                        <button
                                            ...
                                            className={`flex items-center space-x-1 p-2 rounded-full transition duration-200
                                                        ${(post.likes || []).includes(userId) ? 'text-red-500 bg-red-100' : 'text-gray-500 hover:text-red-500 hover:bg-gray-100'}`}
                                            title={(post.likes || []).includes(userId) ? 'Unlike this post' : 'Like this post'}
                                            disabled={loadingAuth}
                                        >
                                            <i className={`fas fa-heart ${(post.likes || []).includes(userId) ? 'text-red-500' : ''}`}></i>
                                    ...
                                    <div className="mt-4 border-t border-gray-100 pt-4">
                                            ...
                                            <ul className="space-y-2">
                                                {post.comments.map(comment => (
                                                    <li key={comment.id} className="bg-gray-50 p-2 rounded-lg text-sm border border-gray-200">
                                                        ...
                                                        {comment.imageUrl && (
                                                            <img src={comment.imageUrl} alt="Comment attachment" className="mt-2 rounded-md max-w-full h-auto" />
                                                        )}
                                        ...
                                        <div className="flex flex-col mt-3 gap-2">
                                            {commentImages[post.id] && (
                                                <div className="mb-2 p-2 bg-blue-100 rounded-lg flex items-center justify-between shadow-inner">
                                                    <img src={commentImages[post.id]} alt="Comment image preview" className="h-12 w-12 object-cover rounded-md mr-2" />
                                                    <span className="text-xs text-gray-700 truncate">Image selected</span>
                                                    <button onClick={() => clearCommentImage(post.id)} className="ml-2 text-red-500 hover:text-red-700">
                                                        <i className="fas fa-times-circle"></i>
                                                    </button>
                                                </div>
                                            )}
                                            <div className="flex gap-2">
                                                <input
                                                    type="text"
                                                    className="flex-grow p-2 border border-gray-300 rounded-l-lg text-sm focus:ring-1 focus:ring-blue-500"
                                                    ...
                                                    disabled={loadingAuth}
                                                />
                                                <label htmlFor={`commentImageUpload-${post.id}`} className="p-2 bg-gray-400 hover:bg-gray-500 text-white rounded-lg cursor-pointer transition duration-300 ease-in-out flex-shrink-0" title="Upload Image for Comment">
                                                    <i className="fas fa-image"></i>
                                                    <input
                                                        type="file"
                                                ...
                                                <button
                                                    ...
                                                    className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-r-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 flex-shrink-0"
                                                    disabled={loadingAuth || (!commentInputs[post.id]?.trim() && !commentImages[post.id])}
                                                >
                                                    Comment
                                                </button>
                                            </div>
...
const HomePage = ({ setCurrentPage }) => {
    ...
    const hasGreetedThisSession = useRef(false); // To ensure "Welcome back" is shown only once per browser session after login.

    const motivationalMessages = [
        "Keep up the great work!",
        "What new skill will you master today?",
        "Every step forward is progress!",
        "Ready for your daily dose of wisdom?",
    ...
    useEffect(() => {
        // Clear any existing timer when the effect re-runs
        if (messageTimerRef.current) {
            clearTimeout(messageTimerRef.current);
        }

        if (authReady && userProfile && userProfile.name) {
            let messageToShow;
            if (!hasGreetedThisSession.current) {
                // Only greet on the very first time userProfile is loaded and valid in this session
                messageToShow = `Welcome back, ${userProfile.name}! Let's learn something new today!`;
                hasGreetedThisSession.current = true; // Mark as greeted
            } else {
                // For subsequent visits to Home, show a random message
                const randomIndex = Math.floor(Math.random() * motivationalMessages.length);
                messageToShow = motivationalMessages[randomIndex];
            }

            setRobotMessage(messageToShow);
            ...
            // If auth is ready but no user profile (e.g., logged out), hide message
            setShowRobotMessage(false);
            setRobotMessage('');
            hasGreetedThisSession.current = false; // Reset for next login
        }
    ...
    }, [userProfile, authReady, setCurrentPage]); // setCurrentPage as dependency means it re-runs when switching to Home.
        ...
        <div className="p-8 bg-gradient-to-br from-blue-100 to-white rounded-lg shadow-2xl max-w-5xl mx-auto my-12 transform transition-transform duration-300 pt-20 animate-fade-in">
            <div className="bg-white bg-opacity-90 p-4 rounded-lg"> {/* Added wrapper for readability */}
                ...
                <div className="flex flex-col md:flex-row items-start gap-8 mb-10">
                    ...
                    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 flex-shrink-0 text-center order-1 md:order-2 relative"> {/* Added relative for absolute positioning */}
                        ...
                        {showRobotMessage && (
                            <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-full
                                            bg-blue-600 text-white text-sm px-4 py-2 rounded-lg shadow-lg
                                            z-20 transition-opacity duration-500 ease-in-out"
                                 style={{ opacity: showRobotMessage ? 1 : 0 }}>
                                {robotMessage}
                                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0
                                                ...
                                                border-t-8 border-t-blue-600"></div>
                            </div>
                        )}
                    </div>
